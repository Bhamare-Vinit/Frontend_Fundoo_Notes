...
//   // selectedTab, layoutType, searchQuery,
...
import Box from "@mui/material/Box";
import { useSelector, useDispatch } from "react-redux"; // Import hooks from React-Redux
import { addNote, updateNote, removeNote } from "../redux/noteSlice"; // Import actions
import { Outlet } from "react-router-dom";
...
const Notes = ({ open, setOpen }) => {
  ...
  const handleNewNote = (newNote) => {
    dispatch(addNote(newNote)); // Dispatch addNote action
  };

  const handleNoteUpdate = (updatedNoteId, updatedNoteData) => {
    dispatch(updateNote({ id: updatedNoteId, updatedNoteData })); // Dispatch updateNote action
  };

  const handleNoteRemove = (noteId) => {
    dispatch(removeNote(noteId)); // Dispatch removeNote action
  };
